...
export const criarNovoAgendamento = (req: Request, res: Response) => {
  try {
    ...
    res
      .status(400)
      .json({ erro: error.message || 'Erro ao criar novo agendamento' });
  }
};
...
export const atualizarStatusAgendamento = (req: Request, res: Response) => {
  try {
    ...
    res
      .status(400)
      .json({ erro: error.message || 'Erro ao atualizar novo status' });
  }
};
...
export const listarTodosAgendamentos = (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ erro: 'Ocorreu um erro ao listar os agendamentos.' });
  }
};
...
export const deletarAgendamentosAntigos = (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .send('Ocorreu um erro ao tentar remover os agendamentos antigos');
  }
};
